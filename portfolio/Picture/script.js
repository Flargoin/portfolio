...
window.addEventListener('DOMContentLoaded', function () {
  ...
  Object(_modules_sliders__WEBPACK_IMPORTED_MODULE_1__["default"])('.main-slider-item', 'vertical', '.main-prev-btn', '.main-next-btn', '', '');
...
/* harmony import */ var _clearState__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__(/*! ./clearState */ "./src/js/modules/clearState.js");
...
var forms = function forms(state) {
  ...
  */
  var forms = document.querySelectorAll('form'),
      ...
      upload = document.querySelectorAll('[name = "upload"]'); // Объект с оповещениями
  ...
  forms.forEach(function (item) {
    ...
    item.addEventListener('submit', function (e) {
      e.preventDefault(); // Создаём блок для оповещений
      ...
      }, 400); // Создаём элемент img для визуального оповещения
      ...
      statusMessage.appendChild(textMessage); // Собираем данные из формы и определяем по какому пути их отправить
      ...
      console.log(api); // Вызываем функцию отправки данных и обрабатываем промис

      Object(_services_reqeusts__WEBPACK_IMPORTED_MODULE_6__["postData"])(api, formData).then(function (res) {
        console.log(res);
        ...
        setTimeout(function () {
          ...
          clearInputs();
          Object(_clearState__WEBPACK_IMPORTED_MODULE_7__["default"])(state);
...
var mask = function mask(selector) {
  var setCursorPosition = function setCursorPosition(pos, elem) {
    ...
    elem.focus(); // прописываем функционал для установки курсора
    ...
    if (elem.setSelectionRange) {
      ...
      elem.setSelectionRange(pos, pos); // Если есть createTextRange (практически тоже самое только для корректной работы в IE)
    } else if (elem.createTextRange) {
      // Диапозон на основе элемента создан.
      var range = elem.createTextRange(); // Объединение граничных точек диапозона

      range.collapse(true); // Конечная точка выделения (символ, позиция)

      range.moveEnd('character', pos); // Начальная точка выделения (символ, позиция)

      range.moveStart('character', pos); // Выделение между moveEnd и moveStart и установка курсора.
  ...
  function createMask(event) {
    ...
    val = this.value.replace(/\D/g, ''); // если пользователь ничего не ввёл, то ставить в дефолтное положение
  ...
  var inputs = document.querySelectorAll(selector); // Всем инпутам назначаем обработчики события input, focus, blur
...
var modals = function modals() {
  ...
  function showModalByTime(selector, time) {
    setTimeout(function () {
      var display;
      document.querySelectorAll('[data-modal]').forEach(function (window) {
...
var sliders = function sliders(slides, dir, prev, next) {
  ...
  paused = false; // Получение всех слайдов со страницы

  var items = document.querySelectorAll(slides); // Функция показа определённого слайда
  ...
  showSlides(slideIndex); // Функция перелистывания вперёд или назад на n слайдов
  ...
  try {
    ...
    var nextArrow = document.querySelector(next),
        prevArrow = document.querySelector(prev); // Вешаем обработчик событий для стрелки 'НАЗАД'

    prevArrow.addEventListener('click', function () {
      // Вызываем функцию перелистывания на -1 слайд
      plusSlides(-1); // Добавляем и убираем классы анимаций
  ...
  } catch (e) {} // Функция автоматического перелистования слайдов
  ...
  function activateAnimation() {
    ...
    if (dir === 'vertical') {
      ...
      paused = setInterval(function () {
        // Вызываем функцию перелистывания на 1 слайд
        plusSlides(1); // Добавляем класс анимации
  ...
  activateAnimation(); // Обращаемся к слайдеру и вешаем на него обработчик события "Наведение мыши", и останавливаем анимацию перелистования.
